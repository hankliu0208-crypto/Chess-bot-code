...
# ==============================================================================
# 1. ROBOT CONFIGURATION & INITIAL HOME MOVE
# ==============================================================================
XARM_IP = "192.168.1.222"

# ==============================================================================
# ADJUSTED HOME POSITION (Gives camera a full overhead view of the board)
# ==============================================================================
HOME_X = 290.0  # Slightly pulled back
HOME_Y = 10.0  # Shifted further Left
HOME_Z = 550.0  # Raised higher for wider Field of View!
...
def get_closest_yolo_piece_mm(target_mm_x, target_mm_y, row, col, conf_thresh=0.08, max_search_radius_mm=50.0):
    """
    Runs YOLO and finds the detected piece center closest to the given physical board coordinates (target_mm_x, target_mm_y).
    
    If no pieces are found within max_search_radius_mm, it safely falls back to (target_mm_x, target_mm_y).
    """
    ...
    # 3. Iterate through all detected piece bounding boxes
    for box in results[0].boxes:
        ...
        # Convert to board millimeters via homography
        mm_x, mm_y = flatten_point(box_center_px_x, box_center_px_y)

        # Calculate physical Euclidean distance to target coordinate
    ...
    # 4. Return closest detected piece center, or the target coordinate if nothing detected
...
def is_white_piece_on_square(row, col, block_w=36.6, block_h=36.6):
    """Captures a fresh frame, warps perspective, and checks if a specific

    grid square (row, col) contains a White piece.
    """
    # 1. Flush stale buffer frames and capture fresh frame
    for _ in range(20):
        mainstream.poll_for_frames()
    ...
    # 2. Warp image to flat 293x293 top-down perspective
    warped_frame = cv2.warpPerspective(raw_frame, H_flatten, (293, 293))

    # 3. Calculate pixel boundaries for target square
    x1 = int(col * block_w)
    y1 = int(row * block_h)
    ...
    # 4. Strict White HSV Mask (Filters light board squares)
    ...
    # 6. Contour Detection & Center Point Calculation
    contours, _ = cv2.findContours(
    ...
    for c in contours:
        if cv2.contourArea(c) > 15:
            ...
            if M["m00"] != 0:
                ...
                # Ensure white centroid sits inside the middle 60% of the square
...
def display():
    for i in range(3000000000000000):
        ...
        # Convert physical millimeter coordinates BACK into camera image pixels
